...
public class Login {
	...
	public static List<Login> retrieve(Object... obj){
		List<Login> ins = new ArrayList<Login>();//Collections.synchronizedList(new ArrayList<Login>());
		...
		for(int i=0;i<obj.length;i++){
			...
			if(obj[i] instanceof UserAccessLevel){
	...
	public static Login getUserLogin(){
		...
		try{
			...
			//String sql = "SELECT logid,username,password,lastlogin,useraccesslevelid,isOnline,userdtlsid,logintime,clientip,clientbrowserisactive FROM login WHERE logid=? AND username=?";
			/*
			 * String[] params = new String[2]; params[0] = userid; params[1] = username;
			 */
			//in = Login.retrieve(sql, params).get(0);
