...
public class VictimPerson extends Persons{

	private long id;
	private String dateTrans;
	private int isActive;
	private Timestamp timestamp;
	private Customer victimePerson;
	private Blotters blotters;
	
	public static List<VictimPerson> retrieve(String sqlAdd, String[] params){
		List<VictimPerson> reps = new ArrayList<VictimPerson>();
		
		String tableReps = "vic";
		String tableCit = "ctz";
		String tableBot = "blot";
		
		String sql = "SELECT * FROM victims " + tableReps + ", customer " + tableCit + ", blotters " + tableBot + " WHERE " +
				tableReps + ".victimsid="+ tableCit + ".customerid AND " +
		...
		while(rs.next()){
			...
			try{per.setId(rs.getLong("vicid"));}catch(NullPointerException e){}
			try{per.setDateTrans(rs.getString("vicdate"));}catch(NullPointerException e){}
			try{per.setIsActive(rs.getInt("isactivevic"));}catch(NullPointerException e){}
			try{per.setTimestamp(rs.getTimestamp("timestampvic"));}catch(NullPointerException e){}
	...
	public static VictimPerson save(VictimPerson rep){
		if(rep!=null){
			
			long id = VictimPerson.getInfo(rep.getId() ==0? VictimPerson.getLatestId()+1 : rep.getId());
			LogU.add("checking for new added data");
			if(id==1){
				LogU.add("insert new Data ");
				rep = VictimPerson.insertData(rep, "1");
			}else if(id==2){
				LogU.add("update Data ");
				rep = VictimPerson.updateData(rep);
			}else if(id==3){
				LogU.add("added new Data ");
				rep = VictimPerson.insertData(rep, "3");
			}
	...
	public static VictimPerson insertData(VictimPerson rep, String type){
		...
		int cnt = 1;
		LogU.add("===========================START=========================");
		LogU.add("inserting data into table victims");
		...
		ps.setLong(cnt++, rep.getVictimePerson()==null? 0 : rep.getVictimePerson().getCustomerid());
		ps.setLong(cnt++, rep.getBlotters()==null? 0 : rep.getBlotters().getId());
		
		
		LogU.add(rep.getDateTrans());
		LogU.add(rep.getIsActive());
		LogU.add(rep.getVictimePerson()==null? 0 : rep.getVictimePerson().getCustomerid());
		...
		LogU.add("data has been successfully saved...");
			...
			LogU.add("error inserting data to victims : " + s.getMessage());
		}
		LogU.add("===========================END=========================");
		return rep;
	}
	...
	public void insertData(String type){
		...
		ps.setLong(cnt++, getVictimePerson()==null? 0 : getVictimePerson().getCustomerid());
		ps.setLong(cnt++, getBlotters()==null? 0 : getBlotters().getId());
		
		
		LogU.add(getDateTrans());
		LogU.add(getIsActive());
		LogU.add(getVictimePerson()==null? 0 : getVictimePerson().getCustomerid());
	...
	public static VictimPerson updateData(VictimPerson rep){
		String sql = "UPDATE victims SET "
				+ "vicdate=?,"
				+ "victimsid=?,"
				+ "blotid=?" 
				+ " WHERE vicid=?";
		...
		int cnt = 1;
		LogU.add("===========================START=========================");
		LogU.add("updating data into table victims");
		...
		LogU.add("data has been successfully saved...");
			...
			LogU.add("error updating data to victims : " + s.getMessage());
		}
		LogU.add("===========================END=========================");
		return rep;
	}
	...
	public static long getLatestId(){
		...
		String sql = "";
		try{
		sql="SELECT vicid FROM victims  ORDER BY vicid DESC LIMIT 1";	
		conn = ConnectDB.getConnection();
	...
	public static boolean isIdNoExist(long id){
		...
		try{
		conn = ConnectDB.getConnection();
		ps = conn.prepareStatement("SELECT vicid FROM victims WHERE vicid=?");
		ps.setLong(1, id);
		rs = ps.executeQuery();
	...
	public void delete(){
		...
		Connection conn = null;
		PreparedStatement ps = null;
		String sql = "UPDATE victims set isactivevic=0 WHERE vicid=?";
	...
	public Customer getVictimePerson() {
		return victimePerson;
	}

	public void setVictimePerson(Customer victimePerson) {
		this.victimePerson = victimePerson;
	}
