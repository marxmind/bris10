...
public class Respondent {
	...
	public static List<Respondent> retrieve(String sqlAdd, String[] params){
		List<Respondent> coms = new ArrayList<>();
		
		String tableCom = "kom";
		String tableCase = "ciz";
		
		String sql = "SELECT * FROM respondent " + tableCom + ", cases " + tableCase + " WHERE " +
		...
		while(rs.next()){
			...
			Respondent com = new Respondent();
			try{com.setId(rs.getLong("resid"));}catch(NullPointerException e){}
			try{com.setDate(rs.getString("resdate"));}catch(NullPointerException e){}
			try{com.setIsActive(rs.getInt("resisactive"));}catch(NullPointerException e){}
	...
	public static Respondent save(Respondent cz){
		if(cz!=null){
			
			long id = Respondent.getInfo(cz.getId() ==0? Respondent.getLatestId()+1 : cz.getId());
			LogU.add("checking for new added data");
			if(id==1){
				LogU.add("insert new Data ");
				cz = Respondent.insertData(cz, "1");
			}else if(id==2){
				LogU.add("update Data ");
				cz = Respondent.updateData(cz);
			}else if(id==3){
				LogU.add("added new Data ");
				cz = Respondent.insertData(cz, "3");
			}
	...
	public static Respondent insertData(Respondent cz, String type){
		String sql = "INSERT INTO respondent ("
				+ "resid,"
				+ "resdate,"
				+ "caseid,"
				+ "resisactive)" 
		...
		int cnt = 1;
		LogU.add("===========================START=========================");
		LogU.add("inserting data into table respondent");
		...
		LogU.add("data has been successfully saved...");
			...
			LogU.add("error inserting data to respondent : " + s.getMessage());
		}
		LogU.add("===========================END=========================");
		return cz;
	}
	...
	public static Respondent updateData(Respondent cz){
		String sql = "UPDATE respondent SET "
				+ "resdate=?,"
				+ "caseid=?" 
				+ " WHERE resid=?";
		...
		int cnt = 1;
		LogU.add("===========================START=========================");
		LogU.add("updating data into table respondent");
		...
		LogU.add("data has been successfully saved...");
			...
			LogU.add("error updating data to respondent : " + s.getMessage());
		}
		LogU.add("===========================END=========================");
		return cz;
	}
