...
public class EmailBean implements Serializable{
	...
	private void send() {
		...
		if(isMore) {
			...
			if(getEmailSelected()!=null) {
				e = getEmailSelected();
				Email.deleteEmail(e.getContendId()+".bris");
	...
	public void load(String tab) {
		...
		if(EmailType.INBOX.getName().equalsIgnoreCase(tab)) {
			...
			String sql = " AND msgtype=1 AND isopen=0 AND isdeleted=0 AND personcpy=? ORDER BY timestampmsg DESC LIMIT 10";
			String[] params = new String[1];
			params[0] = getUser().getUserdtlsid()+"";
	...
	private void copyFileToWebPath(Email em){
		...
		String driveFile =  emailPath +  em.getContendId() + ".html";
	...
	public void loadAttachment() {
		...
		try{
  		 ...
  		 File file = new File(emailPath, getAttachment() + ".html");
