...
public class BankRptBean implements Serializable{
	...
	private List<BankChequeTrans> cheques;
	private List<BankChequeRpt> rpts;
	...
	private Map<Long, Chequedtls> mapProcessedCheques;
	...
	private List<BankChequeRpt> bankLoad(List<BankChequeRpt> banks){
		List<BankChequeRpt> rptsbank = new ArrayList<BankChequeRpt>();
		setMapProcessedCheques(new HashMap<Long, Chequedtls>());
	...
	public void clearFlds(){
		...
		setCalendarTo(null);
		setChecks(new ArrayList<Chequedtls>());
		setCheckSelected(new ArrayList<Chequedtls>());
