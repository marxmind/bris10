...
public class CustomerBean implements Serializable {
	...
	public void oncapture(CaptureEvent captureEvent) {
        ...
        try{
        //File img = new File(driveImage);
        //img.delete();
        //System.out.println("deleting " + deleteImg);
        //img = new File(deleteImg);
        //img.delete();
	...
	private void saveFinalImage(){
		...
		if(!"camera".equalsIgnoreCase(getPhotoId())){
			String photoDestination = IMAGE_PATH + ReadConfig.value(Bris.BARANGAY_NAME) + Bris.SEPERATOR.getName() + "photo" + File.separator;
			//check log directory
			...
			File file = new File(IMAGE_PATH + ReadConfig.value(Bris.BARANGAY_NAME) + Bris.SEPERATOR.getName() + getPhotoId() + ".jpg");
	...
	/*public void calculateAge(){
		...
		if(getBirthdate()!=null && !getBirthdate().isEmpty()){
			int year = Integer.valueOf(getBirthdate().split("-")[0]);
			int month = Integer.valueOf(getBirthdate().split("-")[1]);
			int day = Integer.valueOf(getBirthdate().split("-")[2]);
			int age = DateUtils.getCurrentYear() - year;
			
			System.out.println("get Day today : " + DateUtils.getCurrentDay());
			...
			setAge(age);
			System.out.println("year="+ year +" your age " + age);
		}else{
			setAge(0);
		}
	}*/
