...
public class Customer {
	...
	public static String cardNumber(){
		...
		String brgyCode = ReadConfig.value(Bris.IDCODE) + " ";
	...
	public static Customer insertData(Customer cus, String type){
		...
		ps.setLong(cnt++, cus.getUserDtls()==null? 0 : (cus.getUserDtls().getUserdtlsid()==null? 0 : cus.getUserDtls().getUserdtlsid()));
	...
	public static int getRecordedCitizen(String sqlAdd) {
		Connection conn = null;
		PreparedStatement prep = null;
		ResultSet rs = null;
		int result = 0;
		try{
		String sql = "SELECT count(*) as count FROM customer WHERE cusisactive=1 " + sqlAdd;
		conn = ConnectDB.getConnection();
		prep = conn.prepareStatement(sql);	
		rs = prep.executeQuery();
