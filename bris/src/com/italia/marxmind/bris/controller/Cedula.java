...
public class Cedula {
	...
	public static boolean isExistCedula(Customer cus){
		Cedula cedula = null;
		String sql = " AND (ced.cedate>=? AND ced.cedate<=?) AND ced.cisactive=1 AND cuz.customerid=" + cus.getCustomerid();
	...
	public static Cedula loadCedulaIfExist(Customer cus){
		Cedula cedula = null;
		String sql = " AND (ced.cedate>=? AND ced.cedate<=?) AND ced.cisactive=1 AND ced.cstatus=1 AND cuz.customerid=" + cus.getCustomerid();
		String[] params = new String[2];
		params[0] = DateUtils.getCurrentYear() + "-01-01";
		params[1] = DateUtils.getCurrentYear() + "-12-31";
	...
	public static Cedula insertData(Cedula ced, String type){
		...
		ps.setLong(cnt++, ced.getCustomer()==null? 0 : ced.getCustomer().getCustomerid());
		...
		LogU.add(ced.getCustomer()==null? 0 : ced.getCustomer().getCustomerid());
